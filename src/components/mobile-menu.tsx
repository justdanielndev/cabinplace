...
import { Menu, X, Home, Newspaper, Calendar, User, Trophy, ShoppingBag, Users, FolderOpen, Vote } from 'lucide-react';
import { SignOutButton } from './signout-button';
import { useSettings } from '@/hooks/use-settings';
import Link from 'next/link';

interface MobileMenuProps {
  currentPage?: 'dashboard' | 'news' | 'events' | 'store' | 'profile' | 'leaderboard' | 'teams' | 'projects' | 'voting' | 'admin';
}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-[#7d82b8] to-[#6b70a6] rounded-xl border border-[#7d82b8]/20 shadow-lg">
                      <img
                        src={process.env.NEXT_PUBLIC_LOGO_URL || "/mcab.svg"}
                        alt={`${process.env.NEXT_PUBLIC_APP_NAME || "Midnight Cabin"} Logo`}
                        className="w-6 h-6"
                      />
                    </div>
                    <div>
                      <h1 className="text-xl font-bold text-white">{process.env.NEXT_PUBLIC_APP_NAME || "Midnight Cabin"}</h1>
                    </div>
                  </div>
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    {settings.VotingEnabled && (
                      <Link href="/voting" className={`flex items-center justify-center space-x-4 font-medium hover:text-[#7d82b8] transition-colors py-6 text-xl ${
                        isActive('voting') ? 'text-white' : 'text-zinc-400 hover:text-white'
                      }`} onClick={() => setIsOpen(false)}>
