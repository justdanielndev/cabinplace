...
export function DashboardContent() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (dataRes.ok) {
          ...
          if (data.dateOrder && Array.isArray(data.dateOrder)) {
            setDayOrder(data.dateOrder);
          }
        }
  ...
  useEffect(() => {
    if (hackathonData) {
      const updateTimer = () => {
        ...
        const endTimeStr = hackathonData['end-date-and-time'];
        const startTimeStr = hackathonData['start-date-and-time'];
        
        if (!endTimeStr || !startTimeStr) return;
        
        const startTime = new Date(startTimeStr.replace(/\.(\d{3})Z$/, 'Z')).getTime();
        const endTime = new Date(endTimeStr.replace(/\.(\d{3})Z$/, 'Z')).getTime();
  ...
  const getUpcomingEvents = () => {
    ...
    const dayDates = {
      ...
      'Tuesday': hackathonData.tuesday,
      'Wednesday': hackathonData.wednesday,
      'Thursday': hackathonData.thursday,
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-zinc-400 text-sm cursor-default">
                  {hackathonData && new Date().getTime() < new Date(hackathonData['start-date-and-time'].replace(/\.(\d{3})Z$/, 'Z')).getTime() 
